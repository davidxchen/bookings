...
import (
	...
	"github.com/davidxchen/bookings/internal/repository/dbrepo"
	"net/http"
	"strconv"
	"strings"
	"time"
...
func (m *Repository) Reservation(w http.ResponseWriter, r *http.Request) {
	...
	sd := res.StartDate.Format(defaultDateLayout)
	ed := res.EndDate.Format(defaultDateLayout)
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	startDate, err := time.Parse(defaultDateLayout, sd)
	if err != nil {
	...
	endDate, err := time.Parse(defaultDateLayout, ed)
	if err != nil {
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	startDate, err := time.Parse(defaultDateLayout, start)
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't parse start date!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	endDate, err := time.Parse(defaultDateLayout, end)
	if err != nil {
	...
	_ = render.Template(w, r, "choose-room.page.gohtml", &models.TemplateData{
		Data: data,
	})
}
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	startDate, _ := time.Parse(defaultDateLayout, sd)
	endDate, _ := time.Parse(defaultDateLayout, ed)

	roomID, _ := strconv.Atoi(r.Form.Get("room_id"))

	available, err := m.DB.SearchAvailabilityBy(startDate, endDate, roomID)
	if err != nil {
...
func (m *Repository) ReservationSummary(w http.ResponseWriter, r *http.Request) {
	...
	sd := reservation.StartDate.Format(defaultDateLayout)
	ed := reservation.EndDate.Format(defaultDateLayout)
	stringMap := make(map[string]string)
	stringMap["start_date"] = sd
	stringMap["end_date"] = ed
